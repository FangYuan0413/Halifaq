...
- `components/DailyLifeMikuButton.tsx` — a one-off Miku-theme reskin of the "Daily Life" category link (sidebar and mobile pills): a wave/beach button image (`public/miku-daily-life-button.png`) with a small chibi Miku badge (`public/miku-coconut.png`, both AI-generated for this project and cropped/trimmed with ImageMagick) peeking from the top-right corner, and a "Off to Daily Life~ 🌊" toast on click. Only swapped in when the Miku theme is active; every other category stays a plain link.
