...
- `app/forgot-password/page.tsx` + `components/ForgotPasswordForm.tsx` — password recovery via a 6-digit emailed code instead of a magic link: enter your email (`supabase.auth.resetPasswordForEmail`), then enter the code plus a new password (`supabase.auth.verifyOtp({ type: "recovery" })` followed by `supabase.auth.updateUser({ password })`). Linked from a "Forgot password?" link on the login form. **Requires a one-time dashboard step** — Supabase's default "Reset Password" email template sends a clickable link, not a code. In the Supabase dashboard, go to Authentication → Email Templates → Reset Password, and make sure the template includes `{{ .Token }}` (that's the 6-digit code) rather than only `{{ .ConfirmationURL }}`. Until that's edited, this flow will send an email but the code inside won't work as expected.
- `components/AuthForm.tsx` — a hero above the card shows the logo's icon mark large, with three water-ripple rings expanding outward from behind it on a loop; the toggle Log In / Sign Up form itself is wired to real Supabase auth (`supabase.auth.signInWithPassword` / `supabase.auth.signUp`), with error/info messages, and redirects to `/feed` on success. A failed login also shows a "Resend confirmation email" link (`supabase.auth.resend`) — Supabase's "Invalid login credentials" message covers both a wrong password and an unconfirmed email address, so rather than guessing which one it is, the user gets a one-click way to trigger a fresh confirmation email in case that's the actual problem.
...
- `components/EventsMikuButton.tsx` — same reskin pattern for the "Events" category link: a cork bulletin-board button (`public/miku-events-button.png`) with a small chibi Miku (`public/miku-peeking.png`) peeking curiously over its top edge, hands gripping the button directly (the source art's placeholder ledge was removed via ImageMagick flood-fill so nothing floats above the real edge), a question mark beside her, and a "Off to Events~ 📌" toast on click. All three Miku category buttons (`Daily Life`/`Education`/`Events`) share a click-tap animation — a quick squash-and-hop on Miku herself (`.animate-miku-tap` in `globals.css`) plus a subtle `active:scale-95` press on the whole button.
